    ...
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        ...
        let result = unsafe {
            (CONPTY.CreatePseudoConsole)(
                ...
                // PSUEDOCONSOLE_INHERIT_CURSOR removed: it sends a cursor
                    // position query (DSR) on the output pipe and blocks waiting
                    // for a response on the input pipe, causing a deadlock when
                    // the reader thread hasn't set up bidirectional forwarding.
