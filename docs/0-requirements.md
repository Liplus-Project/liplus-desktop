...
### Li+ Desktop本体
- デスクトップモード: Tauri組み込みアップデーター（GitHub Releasesからチェック・ダウンロード・適用）
- Webモード: バイナリ差し替え（手動 or パッケージマネージャ）
- 起動時にアップデート確認、通知のみ（強制アップデートしない）

### Li+プログラム（アダプター層・タスク層・オペレーション層）
- 実装方式未決定（Rustコード化 / 設定ファイル駆動 / その他）
- Li+core.mdの同梱方式・更新方式もこれに依存
- 方式が決まるまではアプリリリースに同梱する暫定方針
- Linuxカーネル方式（多少遅れてもよい）を想定

### CLI
- 各CLIのアップデートはCLI側の仕組みに委ねる（アプリは介入しない）
- CLIバージョン表示はUI上に出せるとよい（情報表示のみ）
